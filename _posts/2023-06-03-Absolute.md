...
Absolute es una máquina dificil que se centra principalmente en la enumeración y obtención de credenciales hasta lograr obtener una shell en el sistema. En este proceso, exploraremos ataques comunes como el AS-REP Roasting, contraseñas en texto plano y también utilizaremos Wireshark para realizar análisis dinámico.

Utilizaremos Bloodhound para enumerar el dominio en busca de posibles rutas de ataque, aprovechándonos de permisos que nos permiten añadir Shadow Credentials y obtener acceso remoto al sistema. Además, haremos uso de Kerberos Relay para añadir nuestro usuario actual al grupo de Administradores. Finalmente, aprenderemos cómo mitigar este tipo de ataques.
...
Con la utilidad extractports, copiamos los puertos a la clipboard
```bash
